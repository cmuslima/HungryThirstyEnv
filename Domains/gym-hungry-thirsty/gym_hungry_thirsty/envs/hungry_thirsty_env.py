...
class HungryThirstyEnv(gym.Env):
    ...
    def __init__(self, size=(6, 6), seed=47, dir=None, initial_position=None):
        """
        ...
        self.window_closed = False  # A variable to track if the window was closed
        self.frames = []
        ...
        home_dir = os.path.expanduser('~') #need to fix this, not accurate path


        cwd = os.getcwd()

        self.FOOD_IMG_IPY = ipywidgets.Image.from_file(f"/{dir}/VisualAssets/PNG/soup_small.png")
        self.WATER_IMG_IPY = ipywidgets.Image.from_file(f"/{dir}/VisualAssets/PNG/water_small.png")
        self.AGENT_IMG_IPY = ipywidgets.Image.from_file(f"/{dir}/VisualAssets/PNG/cute_small.png")
        #self.AGENT_EAT_IMG_IPY = ipywidgets.Image.from_file(f"/{dir}/VisualAssets/PNG/agent_eat.png")

        self.FOOD_IMG = Image.open(f"{dir}/VisualAssets/PNG/soup_small.png")
        self.WATER_IMG = Image.open(f"{dir}/VisualAssets/PNG/water_small.png")
        self.AGENT_IMG = Image.open(f"{dir}/VisualAssets/PNG/cute_small.png")
        #self.AGENT_EAT_IMG = Image.open(f"{dir}/VisualAssets/PNG/cute_small.png")
    ...
    def reset(self, state_as_np_array=False, food_loc=None, water_loc=None, initial_state=None,
        ...
        # print(f'food location = {self.food_loc}')
        # print(f'water location = {self.water_loc}')
        # print(f'start state = {self.state["position"]}')
    ...
    def save_frame(self):
        ...
        x = self.canvas_root.winfo_rootx() + self.render_tracking["canvas"].winfo_x()
        y = self.canvas_root.winfo_rooty() + self.render_tracking["canvas"].winfo_y()
        x1 = x + self.render_tracking["canvas"].winfo_width()
        y1 = y + self.render_tracking["canvas"].winfo_height()

        # Grab the canvas content as an image
        frame = ImageGrab.grab(bbox=(x, y, x1, y1))
        self.frames.append(frame)

    def save_gif(self, gif_filename="output.gif", duration=100):
    ...
    def render_with_episode(self, state_counter=None, action=None, mode=None, score=None, episode=None, timestep=None):
        """
        ...
        def close_window():
            ...
            if self.canvas_root:
                ...
                close = messagebox.askyesno("Exit", "Are you sure you want to stop the rendering?")
                if close:
                    self.canvas_root.quit()
                    self.canvas_root.destroy()
                    self.canvas_root = None
                    self.window_closed = True  # Set flag to indicate window was closed
            return close
        ...
        if self.canvas_root is None:
            ...
            self.canvas_root = Tk()
        # Bind the window close event to our handler
        self.canvas_root.protocol("WM_DELETE_WINDOW", close_window)
        #print('window_closed inside render', self.window_closed)
        ...
        if not self.render_tracking:
            ...
            # if action == 'eat':
            #     self.render_tracking["agent_photoImg"] = ImageTk.PhotoImage(self.AGENT_EAT_IMG)
            #     self.render_tracking["agent_obj"] = self.render_tracking["canvas"].create_image(
            #         self.state["position"][0] * self.CELL_WIDTH +
            #         int(self.CELL_WIDTH / 2) - int(self.AGENT_EAT_IMG.width / 2) + self.OFFSET,
            #         self.state["position"][1] * self.CELL_HEIGHT +
            #         int(self.CELL_HEIGHT / 2) - int(self.AGENT_EAT_IMG.height / 2) + self.OFFSET,
            #         anchor=NW,
            ...
            # # Score text - positioned closer to the top-right corner
            # self.render_tracking["score_text"] = self.render_tracking["canvas"].create_text(
            #     self.CANVAS_WIDTH - self.OFFSET * 10 / 5,  # x-coordinate
            #     self.OFFSET / 4,                         # y-coordinate
            #     fill="black",
            #     font="Times 20",
            #     text='""'
            # )

            # Episode text - shifted right to align horizontally with score text and slightly downward
            self.render_tracking["episode_text"] = self.render_tracking["canvas"].create_text(
                self.CANVAS_WIDTH - self.OFFSET * 9 / 5,  # Align x-coordinate with score_text
                self.OFFSET / 4,                         # y-coordinate (slightly lower than score_text)
                fill="black",
                font="Times 25",
                text='"Trajectory: N/A"'
            )
...
###########
        # Update the agent's cell color based on hunger and thirst state
        position = self.state["position"]
        cell_x1 = position[0] * self.CELL_WIDTH + self.OFFSET
        cell_y1 = position[1] * self.CELL_HEIGHT + self.OFFSET
        cell_x2 = cell_x1 + self.CELL_WIDTH
        cell_y2 = cell_y1 + self.CELL_HEIGHT



        # If the agent's cell doesn't exist yet, create a new one
        if "agent_cell" not in self.render_tracking:
            self.render_tracking["agent_cell"] = self.render_tracking["canvas"].create_rectangle(
                cell_x1, cell_y1, cell_x2, cell_y2, fill="", outline=""
            )
        else:
            # Move the rectangle to match the agent's new position
            self.render_tracking["canvas"].coords(self.render_tracking["agent_cell"], cell_x1, cell_y1, cell_x2, cell_y2)

        # Update the cell's color based on the agent's state
        if not self.state["hungry"]:  # If the agent is not hungry, turn the cell green (highest priority)
            if timestep > 0:
                self.render_tracking["canvas"].itemconfigure(self.render_tracking["agent_cell"], fill="green", outline="")
            #print("GREEN!!")
            #input('wait')
            # Add a star emoji on top of the green cell
            # if "star_emoji" not in self.render_tracking:
            #     self.render_tracking["star_emoji"] = self.render_tracking["canvas"].create_text(
            #         (cell_x1 + cell_x2) / 2, (cell_y1 + cell_y2) / 2,  # Centered in the middle of the cell
            #         text="⭐", font=("Arial", 50), fill="yellow"
            #     )

        elif not self.state["thirsty"]:  # If the agent is not thirsty, turn the cell blue
            if timestep > 0:
                self.render_tracking["canvas"].itemconfigure(self.render_tracking["agent_cell"], fill="blue", outline="")
            
            # Remove the star if previously added (if agent is not hungry anymore)
            if "star_emoji" in self.render_tracking:
                self.render_tracking["canvas"].delete(self.render_tracking["star_emoji"])
                del self.render_tracking["star_emoji"]

        else:  # If neither hungry nor thirsty, leave the cell unchanged (transparent)
            self.render_tracking["canvas"].itemconfigure(self.render_tracking["agent_cell"], fill="", outline="")
        ...
        # Render the action text "eat" in the agent's cell if applicable

        if action == 4:
            # Check if the "action_text" exists, if not, create it
            if "action_text" not in self.render_tracking:
                self.render_tracking["action_text"] = self.render_tracking["canvas"].create_text(
                    (cell_x1 + cell_x2) / 2,  # Center horizontally in the cell
                    (cell_y1 + cell_y2) / 2 - 50,  # Slightly above the center of the cell
                    text="eat", font=("Arial", 20, "bold"), fill="red"
                )
            else:
                # Update the position and content of the existing text
                self.render_tracking["canvas"].coords(
                    self.render_tracking["action_text"],
                    (cell_x1 + cell_x2) / 2,
                    (cell_y1 + cell_y2) / 2 - 50 # Adjust the position to be above the agent
                )
                self.render_tracking["canvas"].itemconfigure(
                    self.render_tracking["action_text"], text="eat", fill="red"
                )
        elif action == 5:
            # Check if the "action_text" exists, if not, create it
            if "action_text" not in self.render_tracking:
                self.render_tracking["action_text"] = self.render_tracking["canvas"].create_text(
                    (cell_x1 + cell_x2) / 2,  # Center horizontally in the cell
                    (cell_y1 + cell_y2) / 2 - 50,  # Slightly above the center of the cell
                    text="drink", font=("Arial", 20, "bold"), fill="red"
                )
            else:
                # Update the position and content of the existing text
                self.render_tracking["canvas"].coords(
            ...
            # Remove the text if action is not "eat"
            if "action_text" in self.render_tracking:
                self.render_tracking["canvas"].delete(self.render_tracking["action_text"])
                del self.render_tracking["action_text"]
###########
        ...
        if score is not None:
            self.render_tracking["canvas"].itemconfigure(self.render_tracking["score_text"],
                                                        text="# of times agent is NOT hungry: " + str(score))

        if state_counter is not None:
            # Adjust the bottom position for the text items
            bottom_padding = self.CANVAS_HEIGHT - self.OFFSET + 50 # Adjust based on your layout

            # Create or update the left text at the bottom
            if "score_text_left" not in self.render_tracking:
                self.render_tracking["score_text_left"] = self.render_tracking["canvas"].create_text(
                    10, bottom_padding,  # Left side position
                    text=f"Not Hungry, Not Thirsty: {state_counter['not_hungry_not_thirsty']}\n"
                        f"Not Hungry, Thirsty: {state_counter['not_hungry_thirsty']}",
                    font=("Times", 25),
                    anchor="sw"  # Anchor to bottom-left
                )
            else:
                # Update the left text if it already exists
                self.render_tracking["canvas"].itemconfigure(
                    self.render_tracking["score_text_left"],
                    text=f"Not Hungry, Not Thirsty: {state_counter['not_hungry_not_thirsty']}\n"
                        f"Not Hungry, Thirsty: {state_counter['not_hungry_thirsty']}"
                )

            # Create or update the right text at the bottom
            if "score_text_right" not in self.render_tracking:
                self.render_tracking["score_text_right"] = self.render_tracking["canvas"].create_text(
                    self.CANVAS_WIDTH - 10, bottom_padding,  # Right side position
                    text=f"Hungry, Thirsty: {state_counter['hungry_thirsty']}\n"
                        f"Hungry, Not Thirsty: {state_counter['hungry_not_thirsty']}",
                    font=("Times", 25),
                    anchor="se"  # Anchor to bottom-right
                )
            else:
                # Update the right text if it already exists
                self.render_tracking["canvas"].itemconfigure(
                    self.render_tracking["score_text_right"],
                    text=f"Hungry, Thirsty: {state_counter['hungry_thirsty']}\n"
                        f"Hungry, Not Thirsty: {state_counter['hungry_not_thirsty']}"
                )
        ...
        if episode is not None:
            self.render_tracking["canvas"].itemconfigure(self.render_tracking["episode_text"],
                                                        text="Trajectory: " + str(episode))
    ...
    def update_state(self, state):
        ...
        self.last_position = self.state["position"]
        self.state = state
    def playback(self, trajectory, episode_metadata, trajectory_id):
        """
        ...
        :param trajectory_id: An identifier for the trajectory being played back.
        
        :return: None
            Play back video (render trajectory) and output score and trajectory ID.
        """
        assert ("food_loc" in episode_metadata.keys())
        assert ("water_loc" in episode_metadata.keys())
     
        self.frames = []
        # print('inside playback')
        # print('food', episode_metadata["food_loc"])
        # print('water', episode_metadata["water_loc"])


        # Initialize counters for the states
        state_counter = { "not_hungry_not_thirsty":0,\
        "not_hungry_thirsty": 0,\
        "hungry_thirsty":0, \
        "hungry_not_thirsty": 0}


        #print(f"Playing back trajectory ID: {trajectory_id}")
        #print(trajectory)
        ...
        total_score = 1  # Placeholder for score calculation logic

        self.window_closed = False
        timestep=0
        while len(trajectory) > 0 and self.window_closed == False:
            #print('window_closed', self.window_closed)
            #print('inside the playback for loop')
            self.render_with_episode(state_counter=state_counter,episode=trajectory_id.upper(), action=action, timestep=timestep)
            time.sleep(0.25)
            ...
            # Example scoring logic: increment score for each step where the agent is not hungry or thirsty
            if not state.get("hungry", True):
                total_score += 1
                #print('total score', total_score)
            # Update the counters based on the current state


            if state.get("hungry") == False and state.get("thirsty") == False:
                state_counter['not_hungry_not_thirsty'] += 1
            elif state.get("hungry") and state.get("thirsty") == False:
                state_counter['hungry_not_thirsty'] += 1
            elif state.get("hungry") and state.get("thirsty"):
                state_counter['hungry_thirsty'] += 1
            elif state.get("hungry") == False and state.get("thirsty"):
                state_counter['not_hungry_thirsty'] += 1

            #print('step', self.step_ct, 'state', state, 'action', action)
            timestep+=1

        time.sleep(1.5)
        self.close()
        #self.save_gif(gif_filename=f"trajectory{trajectory_id.upper()}.gif")

        #print(f"Finished playing back trajectory ID: {trajectory_id}")
        #print(f"Score for trajectory ID {trajectory_id.upper()}: {total_score}")
    ...
    def close(self):
        ...
        self.render_tracking = {}
        self.canvas_root = None
        #print('self.render_tracking', self.render_tracking)
...
entry_point='hungry_thirsty_env:HungryThirstyEnv',
)
